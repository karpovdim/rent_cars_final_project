...
public class ChangeEmailCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        try {
            if (userService.updateEmail(user.getId(), email, newEmail, password)) {
                CodeGenerator codeGenerator = CodeGenerator.getInstance();
                String code = codeGenerator.generateCodeToRegistration();
                userService.updateStatus(user.getId(), User.UserStatus.CONFIRMATION_AWAITING);
                userService.updatePasswordForAuthentication(user.getId(), code);
